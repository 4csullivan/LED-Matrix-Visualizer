...
from ulab import uint16 as u_uint16
...
def run():
    ...
    chunkClock = 0
    micBuff = u_array([0] * CHUNK, dtype=u_uint16)

    tempHistory = u_array([0] * CHUNK, dtype=u_uint16)
    ampHistory = u_array([0] * CHUNK, dtype=u_uint16)
    ...
    while True:
        ...
        if chunkClock >= CHUNK:
            ...
            #spectro = vh.calculate_fft(micBuff)
            #print("%0.3f, %0.3f" % (spectro[3], spectro[CHUNK//2-4]))
            sensitivity = math.pow(65536 - pot.value, 14/15)
            scaled, totalMax = vh.scale_data(micBuff, totalMax, sensitivity)
            #print("%0.3f, %0.3f, %0.3f" % (sensitivity, totalMax, pot.value))
            amp = math.ceil(vh.normalized_rms(scaled))
            #print(amp, totalMax)

            ampHistory = u_concat((u_array([amp], dtype=u_uint16), ampHistory[:-1]))
            ...
            if profile == 0:
                ...
                vis.visualize_tornado(ampHistory)
            elif profile == 4:
